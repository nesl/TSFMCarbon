...
def main(args):
    ...
    pacth_len_t = 2
    model = TimesFMModel(seq_len=input_seq_len_t, 
                         horizen_len=forecast_horizon_t, 
                         pacth_len=pacth_len_t, 
                         device="cuda")
    ...
    with torch.no_grad():
        ...
        for timeseries, forecast, input_mask in tqdm(test_loader, total=len(test_loader)):
            ...
            timeseries = timeseries.float()  # torch.Size([8, 13, 512])
            input_mask = input_mask   # torch.Size([8, 512])
            forecast = forecast.float()  # torch.Size([8, 13, 96])

            # historical_data, gt, result_preds = None, None, None
            # if args.model == "chronos":
            # chronos only support univariate time series
            historical_data = timeseries[:, 0, :]  # torch.Size([8, 24])

            result_preds = model(historical_data, prediction_length=forecast_horizon_t)
...
if  __name__ == '__main__':
    ...
    parser.add_argument(
        "--model", type=str, default="timesfm", help="The type of models we are testing (chronos | moment)"
    )

    parser.add_argument(
        "--region", type=str, default="Uruguay", help="The type of data we are testing (building | electricity | electricity_uci | ecobee)"
    )
