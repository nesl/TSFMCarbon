...
class Dataset_Carbon(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        if size == None:
            ...
            self.seq_len = size[0]  # 24
            self.label_len = size[1]  # 12
    ...
    def __read_data__(self):
        ...
        rows_per_day = 24
        total_days = total_rows // rows_per_day

        # Split days: train = 70%, val = 5% of train, test = rest
        train_days = int(total_days * 0.7)
        val_days = int(train_days * 0.01)
        train_days = train_days - val_days  # Adjust train size after taking val from it
        test_days = total_days - (train_days + val_days)

        border1s = [
            0,
            train_days * rows_per_day,
            (train_days + val_days) * rows_per_day
        ]
        border2s = [
            train_days * rows_per_day,
            (train_days + val_days) * rows_per_day,
            (train_days + val_days + test_days) * rows_per_day
        ]
